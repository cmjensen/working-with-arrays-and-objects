...
const secrets = obj => {
  shStr = ''
  for(let key in obj) {
    key.startsWith('sh') ? shStr += obj[key] : key
  }
  return shStr
}
...
for(key in deleteTheBigNumbers) {
  deleteTheBigNumbers[key]  > 100 ? delete deleteTheBigNumbers[key] : key
}
...
const startsWithK = obj => {
  for(let key in obj) {
    key.startsWith('k') ? delete obj[key] : key
  }
  return obj
}
...
const hiddenTreasure = obj => {
  for(let key in obj) {
    obj[key].includes('treasure') ? key : delete obj[key]
  }
  return obj
}
